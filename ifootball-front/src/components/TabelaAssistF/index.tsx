...
const TabelaAssistF = ({mockRankingData, genderId}: tableProps) => {
    return (
            ...
            <tbody>
                {mockRankingData.map((item, index) => (
                    <tr key={index}>
                        ...
                        <td>{item.name}</td>
                        <td>{item.score} {item.score > 1 || item.score == 0 ? 'Assistências' : 'Assistência'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
